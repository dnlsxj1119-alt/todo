...
export default function App() {
  ...
  // 항목을 다른 날짜로 옮기기 (달력 드래그 / 주간뷰 드래그·백로그 이동 공용).
  // - 시간·중요도·카테고리·마감일 등 나머지 값은 그대로 둔다.
  // - 여러 날에 걸친 일정은 종료일도 같은 간격으로 옮긴다.
  //   안 옮기면 종료일이 시작일보다 앞서는 잘못된 상태가 된다(주간뷰 드래그에서 실제로 그랬다).
  // - updateItem 을 거치므로 구글 캘린더에 쓴 일정도 함께 갱신된다.
  const moveItemTo = useCallback((rawId, newDate, newTimeSlot) => {
    const item = items.find(i => String(i.id) === String(rawId));
    if (!item) return;
    const nextDate = newDate || '';
    const nextSlot = newTimeSlot ?? item.timeSlot;
    if (item.date === nextDate && item.timeSlot === nextSlot) return;
    const patch = { date: nextDate, timeSlot: nextSlot };
    if (nextDate && item.date && item.endDate && item.endDate > item.date) {
      const spanDays = Math.round((new Date(item.endDate) - new Date(item.date)) / 86400000);
      patch.endDate = addDays(nextDate, spanDays);
    }
